...
function makeClient(url = "http://localhost:3100", token = "test-token"): HubClient {
...
describe("HubClient.findTribunalWorker — multiple workers", () => {
  it("TC-H21: returns first online worker among multiple", async () => {
    const agents = [
      { id: "w1", name: "tribunal-worker", status: "offline", capabilities: [] },
    ...
    mockFetch.mockImplementation(async (url: string, opts?: RequestInit) => {
      ...
      if (url.includes("/commands/cmd-1")) {
        ...
        return jsonResponse({
          id: "cmd-1",
          status: "completed",
          result: { verdict: "FAIL", issues: [{ severity: "P1", description: "issue" }] },
        });
      }
      return jsonResponse({}, 404);
    });
