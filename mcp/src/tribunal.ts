 ...
 */
export function parseDiffSummary(summaryLine: string): {
  files: number;
  insertions: number;
  deletions: number;
} {
...
export async function prepareTribunalInput(
  ...
  const truncatedDiffStat =
    diffStatLines.length > 100
...
export function classifyTribunalError(
  ...
  if (
    /OOM|out of memory|heap|ENOMEM/i.test(msg) ||
  ...
  if (
    /internal|ECONNREFUSED|ECONNRESET|SIGKILL|SIGSEGV/i.test(msg) ||
  ...
  return new Promise<TribunalVerdict>(resolve => {
    const callback = (err: Error | null, stdout: string, stderr: string) => {
      if (err) {
        ...
        const crashInfo = classifyTribunalError(
          err,
          stderr,
            ...
            issues: [
              {
                severity: 'P0',
                description:
                  '裁决判定 PASS 但未提供任何证据（passEvidence 为空）。PASS 必须逐条举证。',
              },
            ],
            raw: stdout,
          });
...
export async function runTribunalWithRetry(
  ...
  if (hubClient) {
    const hubResult = await tryRunViaHub(
      hubClient,
      digestContent,
      phase,
      digestPath
...
async function tryRunViaHub(
  ...
  try {
    ...
    console.warn(
      `[tribunal] Hub execution failed: ${err instanceof Error ? err.message : String(err)}`
    );
    return null;
  }
}
...
async function runTribunalWithRetryCli(
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    const isParseFailure = result.issues.some(i =>
      PARSE_FAILURE_INDICATORS.some(indicator =>
        i.description.includes(indicator)
      )
    );
    if (isParseFailure) {
...
export async function evaluateTribunal(
  ...
  // 1. Quick pre-checks
  const preCheckError = await runQuickPreCheck(
    phase,
    outputDir,
    projectRoot,
    startCommit
  ...
  const { digestPath, digestContent } = await prepareTribunalInput(
    phase,
    outputDir,
    projectRoot,
    startCommit
...
export async function executeTribunal(
  ...
  const { verdict, crashed } = await runTribunalWithRetry(
    digestContent,
    phase,
    digestPath
  );
  ...
  if (crashed) {
    ...
    return textResult({
      ...
      digestHash,
      mandate:
        '[FALLBACK] 请调用 auto-dev-reviewer subagent 审查上述材料，然后提交 auto_dev_tribunal_verdict。',
    });
  }
...
async function runQuickPreCheck(
  ...
  if (phase === 5) {
    ...
    const result = await validatePhase5Artifacts(
      outputDir,
      testFileCount,
      resultsContent,
      implFileCount
...
export function buildTribunalLog(
  ...
  log += verdict.issues
    .map(
      i => `- [${i.severity}] ${i.description}${i.file ? ` (${i.file})` : ''}`
    )
    .join('\n');
  log += '\n\n';
  ...
  if (verdict.traces?.length) {
    ...
    log += verdict.traces
      .map(
        t =>
          `- ${t.source} → ${t.status}${t.evidence ? ` — ${t.evidence}` : ''}`
      )
      .join('\n');
    log += '\n\n';
  }
