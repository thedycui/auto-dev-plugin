 ...
 */
export function checkDesignDocCompliance(content: string): {
  compliant: boolean;
  missing: string[];
} {
  ...
  if (!acMatches || acMatches.length < 3) {
    missing.push(
      `验收标准不足（需要 ≥3 条 AC-N，当前 ${acMatches?.length ?? 0} 条）`
    );
  }
 ...
 */
async function computeTestFilesHash(projectRoot: string): Promise<string> {
  try {
    const result = await shell(
      "git ls-files | grep -E '\\.(test|spec)\\.(ts|js|tsx|jsx)$|__tests__/' | sort",
      projectRoot,
      10_000
    );
    ...
    const contents = await Promise.all(
      testFiles.map(f => readFileSafe(join(projectRoot, f)))
    );
...
export function parseTribunalResult(toolResult: {
  ...
  return {
    ...
    feedback: translateFailureToFeedback(
      'TRIBUNAL_FAIL',
      parsed.message ?? 'Tribunal process crashed.'
    ),
  };
}
  ...
  if (!approachState) {
    ...
    if (!approaches) {
      return {
        action: 'CONTINUE',
        planFeedback:
          '你的 approach-plan.md 缺少备选方案。请补充至少 1 个与主方案技术路径有本质区别的备选方案（换参数/换 flag 不算，换工具/换思路才算）。',
      };
    }
    approachState = {
      stepId: step,
  ...
  try {
    ...
    if (crashRaw) {
      ...
      if (parsed.crashInfo) {
        ...
        if (ci.stderrFull) {
          const crashLogPath = join(
            outputDir,
            `.tribunal-crash-phase${state.phase}.log`
          );
          await writeFile(
            crashLogPath,
            `[${timestamp}] TRIBUNAL_CRASH category=${ci.errorCategory} exitCode=${ci.exitCode ?? 'N/A'}\n\nSTDERR:\n${ci.stderrFull}\n\nERROR: ${ci.errMessage}\n`,
            'utf-8'
          );
...
async function handleTribunalSubagent(
  ...
  return {
    ...
    escalation: {
      ...
      lastFeedback:
        '裁决已委托给 subagent，请读取 digestPath 文件执行裁决后调用 auto_dev_tribunal_verdict 提交。',
...
async function handleTribunalEscalation(
  ...
  if (escCount >= 2) {
    console.error(
      `[orchestrator] tribunal escalation BLOCKED: step=${currentStep} phase=${phaseKey} escCount=${escCount + 1}`
    );
  ...
  console.error(
    `[orchestrator] tribunal escalation regress: step=${currentStep} phase=${phaseKey} escCount=${escCount + 1}`
  );
  await sm.atomicUpdate({
    phase: 3,
    status: 'IN_PROGRESS',
...
export async function validateStep(
  ...
  switch (step) {
    case '1a': {
      ...
      if (!content || content.length < 100) {
        return {
          passed: false,
          feedback:
            'design.md 不存在或内容不足（< 100 字符），请补充完整的设计方案。',
        };
      }
      ...
      if (content) {
        ...
        if (/\b(?:REJECT|NEEDS_REVISION)\b/i.test(conclusionText)) {
          const feedbackMatch = content.match(
            /##\s*(?:反馈|Feedback|问题|Issues|P0|P1)\s*\n([\s\S]*?)(?=\n##|$)/i
          );
          const feedback =
        ...
        if (!conclusionMatch) {
          ...
          if (/\b(?:REJECT|NEEDS_REVISION)\b/i.test(content)) {
            return {
              passed: false,
              feedback:
                "design-review.md 缺少 ## 结论 段落，且正文包含 REJECT/NEEDS_REVISION 关键词。请确保 review 文件末尾有 '## 结论\\nPASS' 或 '## 结论\\nNEEDS_REVISION'。",
            };
          }
        }
      }
    ...
    case '1c': {
      ...
      const previousHash1c =
        (state.lastArtifactHashes ?? {})['design.md'] ?? '';
      const currentHash1c = hashContent(content1c);
      if (previousHash1c && currentHash1c === previousHash1c) {
        return {
          passed: false,
          feedback:
            'design.md 内容未发生变化（hash 未改变），请根据审查反馈修订设计方案。',
        };
      }
      return { passed: true, feedback: '' };
    }
        ...
        if (!conclusionMatch) {
          ...
          if (/\b(?:REJECT|NEEDS_REVISION)\b/i.test(content)) {
            return {
              passed: false,
              feedback:
                "plan-review.md 缺少 ## 结论 段落，且正文包含 REJECT/NEEDS_REVISION 关键词。请确保 review 文件末尾有 '## 结论\\nPASS' 或 '## 结论\\nNEEDS_REVISION'。",
            };
          }
        }
      }
    ...
    case '2c': {
      ...
      if (previousHash2c && currentHash2c === previousHash2c) {
        return {
          passed: false,
          feedback:
            'plan.md 内容未发生变化（hash 未改变），请根据审查反馈修订实施计划。',
        };
      }
      return { passed: true, feedback: '' };
    }
    ...
    case '3': {
      ...
      if (state.startCommit) {
        ...
        if (diffResult3.exitCode === 0 && diffResult3.stdout.trim() === '') {
          return {
            passed: false,
            feedback:
              '未检测到代码变更（git diff 为空）。Phase 3 要求提交实际代码实现，不能只写文档。',
          };
        }
      }
      // Build + test (with pre-existing failure detection)
      const buildFail3 = await checkBuildWithBaseline(
        buildCmd,
        projectRoot,
        state.startCommit,
        'BUILD_FAILED',
      ...
      const testFail3 = await checkBuildWithBaseline(
        testCmd,
        projectRoot,
        state.startCommit,
        'TEST_FAILED',
    ...
    case '4a': {
      ...
      const buildFail4 = await checkBuildWithBaseline(
        buildCmd,
        projectRoot,
        state.startCommit,
        'BUILD_FAILED',
      ...
      const testFail4 = await checkBuildWithBaseline(
        testCmd,
        projectRoot,
        state.startCommit,
        'TEST_FAILED',
      ...
      const eval4 = await evaluateTribunal(
        projectRoot,
        outputDir,
        4,
        topic,
    ...
    case '5a': {
      const hasTestCases = await fileExists(
        join(outputDir, 'e2e-test-cases.md')
      );
      if (!hasTestCases) {
        return {
          passed: false,
          feedback:
            'e2e-test-cases.md 不存在。Phase 5a 要求输出测试用例设计文件。',
        };
      }
      return { passed: true, feedback: '' };
    }
    ...
    case '5b': {
      ...
      const eval5 = await evaluateTribunal(
        projectRoot,
        outputDir,
        5,
        topic,
    ...
    case '6': {
      ...
      if (acContent) {
        ...
        const progressLog =
          (await readFileSafe(join(outputDir, 'progress-log.md'))) ?? '';
        ...
        const bindings = await discoverAcBindings(
          effectiveCodeRoot,
        ...
        if (coverage.missing.length > 0) {
          return {
            passed: false,
            feedback:
              `[BLOCKED] Test-bound AC missing bindings: ${coverage.missing.join(', ')}. ` +
      ...
      const eval6 = await evaluateTribunal(
        projectRoot,
        outputDir,
        6,
        topic,
    ...
    case '7': {
      ...
      const retroContent = await readFileSafe(
        join(outputDir, 'retrospective.md')
      );
      if (!retroContent || retroContent.split('\n').length < 30) {
        return {
          passed: false,
          feedback:
            'retrospective.md 不存在或内容不足（< 30 行），请补充完整的复盘报告。',
        };
      }
      return { passed: true, feedback: '' };
    }
    ...
    case '8a': {
      ...
      try {
        ...
        return {
          passed: false,
          feedback: `git 命令执行异常: ${(err as Error).message}`,
        };
      }
      return { passed: true, feedback: '' };
    }
    ...
    case '8b': {
      const buildResultContent = await readFileSafe(
        join(outputDir, 'ship-build-result.md')
      );
      if (!buildResultContent || !buildResultContent.includes('SUCCEED')) {
        return {
          passed: false,
          feedback:
            "ship-build-result.md 不存在或不含 'SUCCEED'，请确认构建成功后写入结果。",
        };
      }
      return { passed: true, feedback: '' };
    }
    ...
    case '8c': {
      const deployResultContent = await readFileSafe(
        join(outputDir, 'ship-deploy-result.md')
      );
      if (!deployResultContent || !deployResultContent.includes('SUCCEED')) {
        return {
          passed: false,
          feedback:
            "ship-deploy-result.md 不存在或不含 'SUCCEED'，请确认部署成功后写入结果。",
        };
      }
      return { passed: true, feedback: '' };
    }
    ...
    case '8d': {
      const verifyContent = await readFileSafe(
        join(outputDir, 'ship-verify-result.md')
      );
      if (!verifyContent) {
        return {
          passed: false,
      ...
      if (verifyContent.includes('CODE_BUG')) {
        return {
          passed: false,
          feedback:
            '远程验证发现代码问题（CODE_BUG），需要回退到 Phase 3 修复。',
          regressToPhase: 3,
        };
      }
      ...
      return {
        passed: false,
        feedback:
          '远程验证失败（ENV_ISSUE 或其他环境问题），需要人工介入排查环境。',
      };
    }
...
export async function buildTaskForStep(
  ...
  if (step === '3') {
    ...
    let reviewSection = '';
    const planReviewContent = await readFileSafe(
      join(outputDir, 'plan-review.md')
    );
    ...
    if (designContent3) {
      const summaryMatch = designContent3.match(
        /## (?:概述|Summary)\s*\n([\s\S]*?)(?=\n## |$)/
      );
      const snippet =
        summaryMatch?.[1]?.trim() ??
...
async function handlePhaseRegress(
  ...
  console.error(
    `[orchestrator] phase regress: step=${currentStep} regressTo=${validation.regressToPhase} round=${currentShipRound}`
  );
...
async function handleCircuitBreaker(
  ...
  if (approachResult.action === 'CIRCUIT_BREAK') {
    console.error(
      `[orchestrator] circuit breaker: step=${currentStep} phase=${phaseForStep(currentStep)}`
    );
    await sm.atomicUpdate({
      stepIteration: 0,
...
async function handleValidationFailure(
  ...
  if (validation.tribunalResult) {
    ...
    if (validation.tribunalResult.subagentRequested) {
      return handleTribunalSubagent(
        ctx,
        phaseKey,
        count,
        currentStep,
        validation.tribunalResult
      );
    }

    if (
      validation.tribunalResult.rawParseFailure &&
      validation.tribunalResult.rawOutput
    ) {
      return handleTribunalParseFailure(
        ctx,
        phaseKey,
        count,
        currentStep,
    ...
    if (validation.tribunalResult.crashed) {
      console.error(
        `[orchestrator] tribunal crashed: step=${currentStep} phase=${phaseKey}`
      );
      return handleTribunalCrashEscalation(
        ctx,
        phaseKey,
        count,
        currentStep,
    ...
    if (count >= 3) {
      console.error(
        `[orchestrator] tribunal failure limit reached: step=${currentStep} phase=${phaseKey} count=${count}`
      );
      return handleTribunalEscalation(
        ctx,
        phaseKey,
        currentStep,
        validation.feedback
  ...
  const { result: circuitResult, approachAction: approachResult } =
    await handleCircuitBreaker(ctx, stepState, currentStep, validation);
  if (circuitResult) return circuitResult;

  // Check iteration limit (skip if approachState exists)
  const hasApproachState = !!(
    (approachResult.action === 'CONTINUE' && approachResult.approachState) ||
  ...
  const effectiveStep =
    revisionStep !== currentStep ? revisionStep : currentStep;
  ...
  const attemptSummary = buildPreviousAttemptSummary(
    effortKey,
    updatedEffortRetry,
    validation.feedback
  );
...
async function advanceToNextStep(
  ...
  const REVISION_TO_PARENT: Record<string, string> = {
    '1c': '1b',
    '2c': '2b',
    '5c': '5b',
  };
  ...
  if (parentStep) {
    ...
    if (newRevisionCycles >= EFFORT_LIMITS.maxRevisionCycles) {
      ...
      await sm.appendToProgressLog(
        '\n' +
          sm.getCheckpointLine(
            currentPhase,
            undefined,
            'BLOCKED',
            `Revision cycles exhausted for ${parentStep} (${newRevisionCycles}/${EFFORT_LIMITS.maxRevisionCycles}).`
          ) +
          '\n'
      );
      return {
    ...
    await sm.appendToProgressLog(
      '\n' +
        sm.getCheckpointLine(
          ...
          undefined,
          'PASS',
          `Revision step ${currentStep} passed. Re-validating ${parentStep} (revision cycle ${newRevisionCycles}/${EFFORT_LIMITS.maxRevisionCycles}).`
        ) +
        '\n'
    );
    await sm.atomicUpdate({
  ...
  if (
    ...
    if (planContent) {
      ...
      const taskNums = taskMatches.map(m =>
        parseInt(m.replace(/^## Task\s+/, ''), 10)
      );
...
export async function computeNextTask(
  ...
  const effectiveRoot =
    (state.worktreeRoot as string | null | undefined) ?? projectRoot;
  ...
  const effectiveCodeRoot = state.worktreeRoot
    ? state.codeRoot
      ? join(
          state.worktreeRoot as string,
          relative(projectRoot, state.codeRoot as string)
        )
      : (state.worktreeRoot as string)
    : (state.codeRoot ?? projectRoot);

  // Ship extra variables for Phase 8 prompt rendering
  const shipExtraVars: Record<string, string> | undefined =
    state.ship === true
      ? {
