...
auto_dev_complete()                       # 完成门禁，返回 timingSummary + tokenUsage
```
...
1. 调用 `auto_dev_init(projectRoot, topic, mode, interactive?, dryRun?, skipE2e?)` → 获取技术栈和变量
   ...
   - `tdd=true`：Phase 3 每个 task 执行 RED-GREEN-REFACTOR（先写失败测试再实现）
...
**TDD Iron Law（--tdd 模式）：禁止在没有失败测试的情况下编写实现代码。**
...
1. 调用 `auto_dev_complete(projectRoot, topic)` — **必须调用，这是完成门禁**
   - 工具会验证所有必需 Phase 是否已 PASS（`skipE2e=true` 时不要求 Phase 5）
   - **实际执行 build_cmd + test_cmd 验证**（编译通过 ≠ 验证通过，Phase 状态 PASS ≠ 最终可用）
   - 如果构建或测试失败 → 返回 error → **必须修复后重新调用**
   - 全部通过 → 自动触发 **Phase 7 RETROSPECTIVE**：
     - 从 progress-log 和产出文档中提取经验教训
     - 标记 reusable=true 的经验提升到全局 lessons-global.json
     - 生成 retrospective.md 回顾总结
   - 返回 `canComplete=true` + `timingSummary` + `tokenUsage` + `retrospective`
...
Phase 3 每个 task 执行 RED-GREEN-REFACTOR 循环。Phase 5 从"写全部测试"变为"补充集成/E2E 测试"。
