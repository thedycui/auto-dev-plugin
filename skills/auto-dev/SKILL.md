...
- 必须有新增的测试文件（checkpoint 通过 git diff 检测 *Test.java / *.test.ts 等）
...
5. `auto_dev_checkpoint(phase=5, status=result)` — **checkpoint 会验证测试文件和执行结果的存在**
...
1. 调用 `auto_dev_complete(projectRoot, topic)` — **必须调用，这是完成门禁**
   ...
   - 成功时还返回 `timingSummary`（每个 Phase 耗时）和 `tokenUsage`（token 消耗汇总）
