...
async function seedLocal(entries: LessonEntry[]): Promise<void> {
  await writeFile(
    join(outputDir, 'lessons-learned.json'),
...
async function readLocal(): Promise<LessonEntry[]> {
  return JSON.parse(
    await readFile(join(outputDir, 'lessons-learned.json'), 'utf-8')
  );
}
...
describe('feedback()', () => {
  it('helpful verdict: score +3, lastPositiveAt updated', async () => {
    ...
    const result = await mgr.feedback([{ id: 'fb-1', verdict: 'helpful' }], {
      phase: 3,
      topic: 'test',
    });
  ...
  it('not_applicable verdict: score -1', async () => {
    ...
    await mgr.feedback([{ id: 'fb-2', verdict: 'not_applicable' }], {
      phase: 3,
      topic: 'test',
    });
  ...
  it('incorrect verdict: score -5', async () => {
    ...
    await mgr.feedback([{ id: 'fb-3', verdict: 'incorrect' }], {
      phase: 3,
      topic: 'test',
    });
  ...
  it('score floor at 0', async () => {
    ...
    await mgr.feedback([{ id: 'fb-4', verdict: 'incorrect' }], {
      phase: 3,
      topic: 'test',
    });
  ...
  it('feedbackHistory capped at MAX_FEEDBACK_HISTORY (20)', async () => {
    ...
    const entry = makeEntry({
      id: 'fb-5',
      score: 10,
      feedbackHistory: existingHistory,
    });
    await seedLocal([entry]);
    const mgr = createManager();

    await mgr.feedback([{ id: 'fb-5', verdict: 'not_applicable' }], {
      phase: 3,
      topic: 'test',
    });
    ...
    expect(entries[0].feedbackHistory![MAX_FEEDBACK_HISTORY - 1].verdict).toBe(
      'not_applicable'
    );
  });
  ...
  it('lesson in both local and global updates both files', async () => {
    ...
    const result = await mgr.feedback([{ id: 'both-1', verdict: 'helpful' }], {
      phase: 3,
      topic: 'test',
    });
...
describe('promoteReusableLessons()', () => {
  ...
  it('dedup prevents double promotion', async () => {
    const entry = makeEntry({
      id: 'reuse-2',
      reusable: true,
      lesson: 'already global',
      score: 5,
    });
    await seedLocal([entry]);
    await seedGlobal([
      makeEntry({ id: 'existing', lesson: 'already global', score: 5 }),
    ]);
    const mgr = createManager();
    ...
    it('promotes reusable entries with score >= minScore (AC-2)', async () => {
      ...
      await crossSeedProject([
        ...
        makeEntry({
          id: 'promo-3',
          score: 8,
          lesson: 'non-reusable high-score',
          reusable: false,
          lastPositiveAt: now.toISOString(),
        }),
      ]);
...
describe('addToProject — prefix dedup', () => {
  it('lesson text with same first 60 chars is treated as duplicate (AC-9)', async () => {
    const prefix60 =
      'For dual-file write operations (local + global), use indepen';
    ...
    const newEntry = makeEntry({
      id: 'dup-prefix-2',
      lesson:
        prefix60 + 'dent writeAtomic + rename for each file independently.',
    });
  ...
  it('lesson text with different first 60 chars is added normally (AC-10)', async () => {
    const existingEntry = makeEntry({
      id: 'diff-prefix-1',
      lesson:
        'Always validate input parameters before calling external APIs to avoid runtime errors.',
    });
    await seedGlobal([existingEntry]);
    const mgr = createManager();
    ...
    const newEntry = makeEntry({
      id: 'diff-prefix-2',
      lesson:
        'Use atomic write operations (write-to-temp + rename) to prevent data corruption.',
    });
  ...
  it('short lesson text (< 60 chars) only uses exact match, not prefix', async () => {
    ...
    const newEntry = makeEntry({
      id: 'short-2',
      lesson: 'Short lesson extended with more text',
    });
    const result = await mgr.addToProject(newEntry);
  ...
  it('retired entries are not considered for dedup', async () => {
    const retiredEntry = makeEntry({
      id: 'retired-dup-1',
      lesson:
        'For dual-file write operations (local + global), use independent writeAtomic calls.',
      retired: true,
      retiredAt: new Date().toISOString(),
      retiredReason: 'score_decayed',
    });
    ...
    const newEntry = makeEntry({
      id: 'retired-dup-2',
      lesson:
        'For dual-file write operations (local + global), use independent writeAtomic + rename.',
    });
    const result = await mgr.addToProject(newEntry);
    expect(result.added).toBe(true);
  });
