...
describe("file_not_exists", () => {
  it("should pass when file does not exist", async () => {
    const criteria: AcceptanceCriterion[] = [{
      id: "AC-1", description: "test", layer: "structural",
      structuralAssertions: [{ type: "file_not_exists", path: "deleted.txt" }],
    }];
    const results = await runStructuralAssertions(criteria, tempDir);
    expect(results["AC-1"]!.passed).toBe(true);
  });
  ...
  it("should fail when file exists", async () => {
    ...
    const criteria: AcceptanceCriterion[] = [{
      id: "AC-1", description: "test", layer: "structural",
      structuralAssertions: [{ type: "file_not_exists", path: "should-not-exist.txt" }],
    }];
    const results = await runStructuralAssertions(criteria, tempDir);
    expect(results["AC-1"]!.passed).toBe(false);
  });
...
describe("file_contains", () => {
  it("should pass when file contains pattern", async () => {
    ...
    const criteria: AcceptanceCriterion[] = [{
      id: "AC-1", description: "test", layer: "structural",
      structuralAssertions: [{ type: "file_contains", path: "config.yml", pattern: "max-retry:\\s*3" }],
    }];
    const results = await runStructuralAssertions(criteria, tempDir);
    expect(results["AC-1"]!.passed).toBe(true);
  });
  ...
  it("should fail when file does not exist", async () => {
    const criteria: AcceptanceCriterion[] = [{
      id: "AC-1", description: "test", layer: "structural",
      structuralAssertions: [{ type: "file_contains", path: "missing.yml", pattern: "anything" }],
    }];
    const results = await runStructuralAssertions(criteria, tempDir);
    expect(results["AC-1"]!.passed).toBe(false);
  });
...
describe("file_not_contains", () => {
  it("should pass when file does not contain pattern", async () => {
    ...
    const criteria: AcceptanceCriterion[] = [{
      id: "AC-1", description: "test", layer: "structural",
      structuralAssertions: [{ type: "file_not_contains", path: "app.ts", pattern: "TODO" }],
    }];
    const results = await runStructuralAssertions(criteria, tempDir);
    expect(results["AC-1"]!.passed).toBe(true);
  });
  ...
  it("should pass when file does not exist", async () => {
    const criteria: AcceptanceCriterion[] = [{
      id: "AC-1", description: "test", layer: "structural",
      structuralAssertions: [{ type: "file_not_contains", path: "missing.ts", pattern: "anything" }],
    }];
    const results = await runStructuralAssertions(criteria, tempDir);
    expect(results["AC-1"]!.passed).toBe(true);
  });
...
describe("config_value", () => {
  it("should pass when JSON config value matches", async () => {
    ...
    const criteria: AcceptanceCriterion[] = [{
      ...
      structuralAssertions: [{
        type: "config_value", path: "config.json", key: "database.port", expectedValue: "5432",
      }],
    }];
    const results = await runStructuralAssertions(criteria, tempDir);
    expect(results["AC-1"]!.passed).toBe(true);
  ...
  it("should fail when key path does not exist", async () => {
    await writeFile(join(tempDir, "config.json"), JSON.stringify({ foo: "bar" }));
    const criteria: AcceptanceCriterion[] = [{
      id: "AC-1", description: "test", layer: "structural",
      structuralAssertions: [{
        type: "config_value", path: "config.json", key: "missing.key", expectedValue: "x",
      }],
    }];
    const results = await runStructuralAssertions(criteria, tempDir);
    expect(results["AC-1"]!.passed).toBe(false);
...
describe("build_succeeds", () => {
  it("should pass when build command succeeds", async () => {
    ...
    const results = await runStructuralAssertions(criteria, tempDir, { buildCmd: "echo ok" });
    expect(results["AC-1"]!.passed).toBe(true);
  });
  ...
  it("should fail when build command fails", async () => {
    ...
    const results = await runStructuralAssertions(criteria, tempDir, { buildCmd: "exit 1" });
    expect(results["AC-1"]!.passed).toBe(false);
  });
...
describe("test_passes", () => {
  it("should pass when test command succeeds", async () => {
    ...
    const results = await runStructuralAssertions(criteria, tempDir, { testCmd: "echo ok" });
    expect(results["AC-1"]!.passed).toBe(true);
  });
  ...
  it("should fail when test command fails", async () => {
    ...
    const results = await runStructuralAssertions(criteria, tempDir, { testCmd: "exit 1" });
    expect(results["AC-1"]!.passed).toBe(false);
  });
});
...
describe("boundary: invalid regex in file_contains", () => {
  it("TC-B-06: should not throw on invalid regex pattern", async () => {
    ...
    const criteria: AcceptanceCriterion[] = [{
      id: "AC-1", description: "test", layer: "structural",
      structuralAssertions: [{ type: "file_contains", path: "test.txt", pattern: "[invalid" }],
    }];
...
describe("boundary: malformed JSON in config_value", () => {
  it("TC-B-07: should return FAIL for malformed JSON config file", async () => {
    ...
    const criteria: AcceptanceCriterion[] = [{
      id: "AC-1", description: "test", layer: "structural",
      structuralAssertions: [{
        type: "config_value", path: "config.json", key: "foo", expectedValue: "bar",
      }],
    }];
    const results = await runStructuralAssertions(criteria, tempDir);
    expect(results["AC-1"]!.passed).toBe(false);
...
describe("boundary: mixed AC layers filtering", () => {
  it("TC-B-13: should only process structural ACs, ignoring test-bound and manual", async () => {
    ...
    const criteria: AcceptanceCriterion[] = [
      {
        id: "AC-1", description: "structural", layer: "structural",
