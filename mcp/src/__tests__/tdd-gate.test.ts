...
describe('buildTestCommand', () => {
  ...
  it('Java: single module test', () => {
    const result = buildTestCommand(
      'Java',
      ['service-mod/src/test/java/BarTest.java'],
      '/project'
    );
    expect(result).toBe(
      'mvn test -Dtest="BarTest" -pl service-mod -DfailIfNoTests=false'
    );
  });
  ...
  it('Java: multi-module — two commands joined with &&', () => {
    const result = buildTestCommand(
      'Java',
      ['mod-a/src/test/java/ATest.java', 'mod-b/src/test/java/BTest.java'],
      '/project'
    );
  ...
  it('TypeScript/JavaScript: vitest command', () => {
    ...
    expect(result).toBe(
      'npx vitest run src/__tests__/foo.test.ts --reporter=verbose'
    );
  });
...
describe('isTddExemptTask', () => {
  ...
  it('returns true when task section contains **TDD**: skip', async () => {
    ...
    const tmpDir = await mkdtemp(
      join((await import('node:os')).tmpdir(), 'tdd-test-')
    );
    try {
      await writeFile(
        join(tmpDir, 'plan.md'),
