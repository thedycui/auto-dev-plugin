...
describe('parseApproachPlan', () => {
  it('parses standard format with primary + 2 alternatives', () => {
    ...
    expect(result![0]).toEqual({
      id: 'primary',
      summary: '使用 vitest mock 进行单元测试',
      failCount: 0,
    });
    expect(result![1]).toEqual({
      id: 'alt-a',
      summary: '使用 jest 进行集成测试',
      failCount: 0,
    });
    expect(result![2]).toEqual({
      id: 'alt-b',
      summary: '使用 mocha + chai 进行端到端测试',
      failCount: 0,
    });
  });
  ...
  it('TC-01: parses standard format with goal + primary + 2 alternatives (AC-1)', () => {
    ...
    expect(result![0]).toEqual({
      id: 'primary',
      summary: '安装 vitest + @vue/test-utils，编写组件单元测试',
      failCount: 0,
    });
    expect(result![1]).toEqual({
      id: 'alt-a',
      summary: '纯 Node.js 脚本，提取核心逻辑函数单独测试',
      failCount: 0,
    });
    expect(result![2]).toEqual({
      id: 'alt-b',
