...
> 日期：2026-03-26
> 更新：2026-04-01（架构适配：Orchestrator 模式主路径、移除 additionalRepos）
> 状态：待实现
...
export async function discoverAcBindings(
  ...
  // 扫描 projectRoot（即 effectiveCodeRoot，传入前已解析）
  const grepResult = await execGrepForAcTags(projectRoot, pattern);
...
export async function runAcBoundTests(
  ...
  // 逐条 AC 独立运行测试，避免同文件多 AC 时 exitCode 误归因
  // AC 数量通常 < 10，独立运行性能可接受
  for (const binding of bindings) {
    const cmd = buildTargetedTestCommand(language, binding.testFile, [binding], projectRoot);

    const { exitCode, stdout, stderr } = await execWithTimeout(cmd, {
      cwd: projectRoot,
      timeout: 120_000,  // 单条 AC 测试最多 2 分钟
    });
...
if (phase === 1 && status === "PASS") {
  ...
  // 如果 acceptance-criteria.json 不存在：
  // - 用户提供现有 design.md（无 AC JSON）→ 向后兼容，不阻断
  // - auto-dev Phase 1 新生成的 design.md → 检查是否包含验收标准表格，若有则要求 AC JSON
  if (!acContent) {
    const designContent = await readFile(join(sm.outputDir, "design.md"), "utf-8");
    const hasAcTable = /\|\s*AC-\d+/.test(designContent);
    const isAutoDevGenerated = !sm.designDocSource;  // 非外部绑定 = auto-dev 自生成
    if (hasAcTable && isAutoDevGenerated) {
      return textResult({
        error: "AC_JSON_MISSING",
        message: "design.md 包含验收标准表格但缺少 acceptance-criteria.json。请同时生成结构化 AC 文件。",
        mandate: "[BLOCKED] auto-dev 生成的设计文档必须同时产出 acceptance-criteria.json。",
      });
    }
  }
}
...
> **架构说明**：当前框架默认运行 Orchestrator 模式，在该模式下 `auto_dev_submit` 被全程拦截（返回 BLOCKED），Phase 6 的执行入口是 `orchestrator.ts` 的 `evaluateStep("6")` case，内部直接调用 `evaluateTribunal`。因此框架自动执行逻辑**主要插入点**在 `orchestrator.ts`。`index.ts` 的 `auto_dev_submit` handler 作为 legacy/非 orchestrator 模式的兜底，同样需要添加。

#### 主路径：`orchestrator.ts` `evaluateStep("6")` case

```typescript
case "6": {
  const acJsonPath = join(outputDir, "acceptance-criteria.json");
  // effectiveCodeRoot: 实际代码目录（可能与 projectRoot 不同）
  ...
  if (acContent) {
    ...
    const acLockMatch = progressLog.match(/<!-- AC_LOCK hash=([a-f0-9]+)/);
    if (acLockMatch) {
      ...
      if (currentHash !== storedHash) {
        return {
          passed: false,
          feedback: "[BLOCKED] acceptance-criteria.json 已被篡改（hash 不匹配）。禁止在 Phase 1 之后修改 AC 定义。",
        };
      }
    }

    // 2. 执行 structural 断言（使用 effectiveCodeRoot）
    const structuralResults = await runStructuralAssertions(
      JSON.parse(acContent).criteria,
      effectiveCodeRoot,
    );

    // 3. 执行 test-bound 测试（使用 effectiveCodeRoot）
    const bindings = await discoverAcBindings(effectiveCodeRoot, state.stack.language);
    // language 从 state.stack.language 获取，testCmd 从 OrchestratorContext (ctx.testCmd) 获取
    const testResults = await runAcBoundTests(bindings, effectiveCodeRoot, state.stack.language, ctx.testCmd);

    // 4. 写入框架结果文件，供 Tribunal 和 acceptance-validator Agent 读取
    ...
    // 5. 若 structural 或 test-bound 有 FAIL，仍调用 acceptance-validator Agent 分析原因，
    //    但跳过 Tribunal（框架 FAIL = 验收失败，无需裁决）
    ...
    if (structuralFails.length > 0 || testFails.length > 0) {
      // 调用 acceptance-validator Agent 分析 FAIL 原因（AC 定义不准 vs 代码缺陷）
      // Agent 产出 acceptance-report.md 帮助开发者定位问题
      await runAcceptanceValidatorAgent(projectRoot, outputDir, topic);
      ...
      return {
        ...
        feedback: [
          ...
          "详细分析见 acceptance-report.md",
        ].filter(Boolean).join("\n"),
      };
    }
  }

  // 6. 进入 Tribunal（manual AC + FAIL 分析）
...
#### 兜底路径：`index.ts` `auto_dev_submit(phase=6)` handler

仅在非 orchestrator 模式下执行。在现有 tribunal 调用之前插入同样的 AC 框架执行逻辑（结构相同，返回格式改为 `textResult`）。
...
| 文件 | 改动 |
|------|------|
| `mcp/src/orchestrator.ts` | **主路径**：`evaluateStep("6")` case 增加 AC 框架自动执行（hash 校验 → structural 断言 → test-bound 测试 → 写 framework-ac-results.json → FAIL 短路） |
| `mcp/src/index.ts` | Phase 1 checkpoint 增加 AC JSON 校验 + hash；`auto_dev_submit(phase=6)` handler 增加同样的框架执行逻辑（legacy/非 orchestrator 模式兜底） |
