...
const MAX_ITERATIONS: Record<number, number> = {
...
export class OrchestratorPhaseRunner {
  ...
  private parseTribunalResult(toolResult: ToolResult): PhaseResult {
...
export async function runOrchestrator(config: OrchestratorConfig): Promise<OrchestratorResult> {
  ...
  for (const phase of phases) {
    ...
    if (phase === 1) {
      result = await executePhaseWithReview(
        runner, sm, phase, iterations,
        () => runner.executeDesign(),
        () => runner.executeDesignReview(),
        config,
      );
    } else if (phase === 2) {
      result = await executePhaseWithReview(
        runner, sm, phase, iterations,
        () => runner.executePlan(),
        () => runner.executePlanReview(),
        config,
      );
    } else if (phase === 3) {
      result = await executePhaseWithRetry(
        runner, sm, phase, iterations,
        () => runner.executeImplementation(),
        config,
      );
    } else if (phase === 4) {
      result = await executePhaseWithRetry(
        runner, sm, phase, iterations,
        async () => runner.executeVerify(sm, await sm.loadAndValidate()),
        config,
      );
    } else if (phase === 5) {
      result = await executePhaseWithRetry(
        runner, sm, phase, iterations,
        async () => runner.executeE2ETest(sm, await sm.loadAndValidate()),
        config,
      );
    } else if (phase === 6) {
      result = await executePhaseWithRetry(
        runner, sm, phase, iterations,
        async () => runner.executeAcceptance(sm, await sm.loadAndValidate()),
        config,
      );
    } else if (phase === 7) {
      result = await executePhaseWithRetry(
        runner, sm, phase, iterations,
        async () => runner.executeRetrospective(sm, await sm.loadAndValidate()),
        config,
      );
    } else {
...
async function executePhaseWithReview(
  runner: OrchestratorPhaseRunner,
  sm: StateManager,
  phase: number,
  iterations: Record<number, number>,
  executeFn: () => Promise<PhaseResult>,
  reviewFn: () => Promise<PhaseResult>,
...
async function executePhaseWithRetry(
  _runner: OrchestratorPhaseRunner,
  _sm: StateManager,
  phase: number,
  iterations: Record<number, number>,
  executeFn: () => Promise<PhaseResult>,
