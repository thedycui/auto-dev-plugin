...
## 十、设计修订（基于 Phase 1 评审反馈）

### 修订 1：buildTestCommand 支持多模块 Maven 项目（P0-1）

**问题**：`mvn test -Dtest="BarTest"` 在多模块项目中不指定 `-pl` 会找不到测试类。
...
case "Java 8":
case "Java":
  // 从路径推导模块：service-tifenbao-metrics/src/test/java/com/foo/BarTest.java
  // → module = "service-tifenbao-metrics", class = "BarTest"
...
### 修订 2：RED 验证接受编译失败（P0-2）

**问题**：Java 中 RED 阶段的测试引用不存在的类，导致编译失败而非测试失败。

**修订**：RED 验证改为"任何非零退出码都是有效的 RED"：

```typescript
// RED 验证逻辑
const result = await execTestCommand(testCmd, projectRoot);

if (result.exitCode === 0) {
  // 所有测试都 PASS → 不是有效的 RED
  return { status: "REJECTED", reason: "所有测试都通过了，RED 不成立。测试必须验证尚未实现的逻辑。" };
}

// exitCode !== 0 → 有效的 RED（无论是编译失败还是测试失败）
const failType = result.stderr.includes("COMPILATION ERROR")
  ? "compilation_error"
  : "test_failure";
...
### 修订 3：明确移除现有 TDD Iron Law 代码（P1-3）

**问题**：现有 `index.ts` 556-619 行的 TDD 检查与新的 `tddTaskStates` 检查可能冲突。

**修订**：文件改动清单中明确标注：

| 文件 | 改动 |
|------|------|
| `mcp/src/index.ts` | **删除** 现有 TDD Iron Law 代码块（约 556-619 行），**替换为** tddTaskStates 检查 |

### 修订 4：tddTaskStates 被下游 Phase 消费（P1-1）

**问题**：`tddTaskStates` 只在 Phase 3 内使用，Phase 4/5/7 不消费。

**修订**：
- Phase 4 tribunal checklist 新增："检查 tddTaskStates，验证每个非 exempt task 都有 RED_CONFIRMED 记录"
- Phase 7 retrospective-data 自动提取 TDD gate 统计：RED 拒绝次数、GREEN 拒绝次数、exempt task 数量
- `tribunal-checklists.ts` Phase 4 checklist 中新增 TDD 验证条目

### 修订 5：文件分类改进（P1-2）

**修订**：采用双重过滤——正向匹配测试文件 + 负向排除实现目录：

```typescript
function isTestFile(filePath: string): boolean {
  const testPatterns = [/[Tt]est\.(java|ts|js|py)$/, /\.test\.(ts|js|tsx|jsx)$/, /\.spec\.(ts|js)$/, /_test\.(go|py)$/];
  const isTestPattern = testPatterns.some(p => p.test(filePath));

  // 允许测试资源文件（fixtures、config）
  const isTestResource = /\.(json|yml|yaml|xml|sql|txt|csv)$/.test(filePath)
...
function isImplFile(filePath: string): boolean {
  ...
  // 只关注源代码文件
  return /\.(java|ts|js|py|go|rs|kt)$/.test(filePath);
}
```

RED 阶段检查逻辑：如果 `git diff` 中有任何 `isImplFile()` 返回 true 的文件 → REJECTED。

### 修订 6：状态改用 enum（P2-3，采纳）
...
| 文件 | 改动 | 说明 |
|------|------|------|
| `mcp/src/index.ts` | 修改 | 新增 `auto_dev_task_red` 和 `auto_dev_task_green` handler；**删除**现有 TDD Iron Law 代码块（~556-619行）；Phase 3 checkpoint 替换为 tddTaskStates 检查 |
| `mcp/src/tdd-gate.ts` | **新增** | RED/GREEN 验证逻辑：文件分类（isTestFile/isImplFile）、测试命令生成（buildTestCommand 含多模块支持）、测试执行（接受编译失败作为 RED）、状态管理 |
| `mcp/src/types.ts` | 修改 | StateJsonSchema 新增 `tddTaskStates` 字段（enum 状态） |
| `mcp/src/phase-enforcer.ts` | 修改 | `isTddExemptTask` 从 plan.md 解析豁免标记 |
| `mcp/src/tribunal-checklists.ts` | 修改 | Phase 4 checklist 新增 TDD 验证条目 |
| `mcp/src/retrospective-data.ts` | 修改 | 新增 TDD gate 统计提取 |
| `skills/auto-dev/SKILL.md` | 修改 | Phase 3 驱动循环改为 RED-GREEN 流程 |
| `skills/auto-dev/prompts/phase3-developer.md` | 修改 | 分为 RED prompt（只写测试）和 GREEN prompt（只写实现） |
