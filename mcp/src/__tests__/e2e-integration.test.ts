...
import { StateManager, extractDocSummary, extractTaskList } from "../state-manager.js";
import { checkIterationLimit, computeNextDirective, validateCompletion } from "../phase-enforcer.js";
...
async function simulateCheckpointHandler(
  ...
  // Idempotency check
  if (await sm.isCheckpointDuplicate(params.phase, params.task, status, summary)) {
  ...
  if (status === "REGRESS") {
    if (!params.regressTo) {
      return { result: { error: "REGRESS requires regressTo parameter" }, earlyReturn: true };
    }
    if (params.regressTo >= params.phase) {
      return {
        result: { error: `regressTo(${params.regressTo}) must be < current phase(${params.phase})` },
        earlyReturn: true,
      };
    }
    if ((state.regressionCount ?? 0) >= 2) {
      return {
        result: { status: "BLOCKED", mandate: "[BLOCKED] Max regression count (2) reached." },
        earlyReturn: true,
      };
    }
  }
  ...
  if (status === "NEEDS_REVISION") {
    ...
    const iterCheck = checkIterationLimit(params.phase, newIteration, state.interactive ?? false);
  ...
  const stateForDirective = status === "REGRESS"
    ...
    : state;
  const nextDirective = computeNextDirective(params.phase, status, stateForDirective, params.regressTo);
...
describe("E2E Integration: REGRESS Flow (P0-1, P1-3)", () => {
  ...
  it("TC-2.1: Valid REGRESS increments regressionCount, resets iteration, returns correct directive", async () => {
    ...
    expect((result as Record<string, unknown>)["mandate"]).toContain("[REGRESS]");
  });
  ...
  it("TC-2.2: Invalid REGRESS (regressTo >= currentPhase) returns error WITHOUT mutating state", async () => {
    ...
    expect((result as Record<string, string>)["error"]).toContain("regressTo(4)");
  ...
  it("TC-2.3: REGRESS at max count returns BLOCKED without mutation", async () => {
    ...
    expect(earlyReturn).toBe(true);
    expect(result).toHaveProperty("status", "BLOCKED");
    expect((result as Record<string, string>)["mandate"]).toContain("Max regression count");
  ...
  it("TC-2.4: Two successive regressions -- first allowed, second's directive is BLOCKED", async () => {
    ...
    const interimLine = sm.getCheckpointLine(4, undefined, "IN_PROGRESS", "back at phase 4");
    await sm.appendToProgressLog("\n" + interimLine + "\n");
    ...
    expect((second.result as Record<string, string>)["mandate"]).toContain("[BLOCKED]");
    ...
    const interimLine2 = sm.getCheckpointLine(4, undefined, "IN_PROGRESS", "back at phase 4 again");
    await sm.appendToProgressLog("\n" + interimLine2 + "\n");
...
describe("E2E Integration: State Rebuild from Progress-Log (AC-2)", () => {
  ...
  it("TC-3.1: Corrupted state.json triggers rebuild from progress-log", async () => {
    ...
    const detectStackSpy = (await import("vitest")).vi.spyOn(sm, "detectStack").mockResolvedValue(TEST_STACK);
  ...
  it("TC-3.2: Dirty state.json -- clear dirty flag to recover", async () => {
    ...
    await writeFile(sm.stateFilePath, JSON.stringify(dirtyState, null, 2), "utf-8");
...
describe("E2E Integration: Checkpoint Pipeline (Entry Point)", () => {
  ...
  it("TC-5.2: Idempotent checkpoint -- duplicate detected and skipped", async () => {
    ...
    const different = await sm.isCheckpointDuplicate(1, undefined, "NEEDS_REVISION", "Design approved");
...
describe("E2E Integration: Negative Test Cases", () => {
  ...
  it("TC-N1: REGRESS without regressTo returns error, no mutation", async () => {
    ...
    expect(earlyReturn).toBe(true);
    expect(result).toHaveProperty("error", "REGRESS requires regressTo parameter");
