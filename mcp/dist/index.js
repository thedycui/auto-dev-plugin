...
const TOPIC_PATTERNS = [
    ...
    {
        pattern: /refactor|重构|优化|性能|perf|cleanup|clean.?up|提升|降低耗时|减少内存/i,
        type: 'refactor',
    },
    // Docs signals
    ...
    // Config signals
    {
        pattern: /\bconfig\b|配置|环境变量|\.env|\.yml|\.yaml|\.properties|settings/i,
        type: 'config',
    },
    // Test-only signals (treat as refactor — skip full design)
    {
        pattern: /补充测试|添加测试|单元测试|集成测试|测试覆盖|test.?coverage|add.?test|write.?test/i,
        type: 'refactor',
    },
];
...
server.tool('auto_dev_init', 'Initialize auto-dev session: create work dir, detect tech stack, init state. If directory exists, onConflict controls behavior (resume/overwrite).', {
    ...
    if (await sm.outputDirExists()) {
            ...
            if (state.worktreeRoot && state.worktreeBranch) {
                ...
                try {
                    ...
                    if (!worktreeDirExists) {
                        const branchOutput = await new Promise(resolve => {
                            execFileGit('git', ['branch', '--list', wtBranch], { cwd: projectRoot, timeout: 10_000 }, (err, stdout) => resolve(err ? '' : stdout || ''));
                        });
                        if (branchOutput.trim()) {
                            await new Promise((resolve, reject) => {
                                execFileGit('git', ['worktree', 'add', wtRoot, wtBranch], { cwd: projectRoot, timeout: 30_000 }, (err) => (err ? reject(err) : resolve()));
                            });
                        }
                    }
                }
        ...
        if (onConflict === 'overwrite') {
            ...
            sm._savedDesignDoc = savedDesignDoc;
            sm._savedProgressLog = savedProgressLog;
        }
    }
    ...
    if (!changeType) {
        ...
        if (!designDocContent) {
            ...
            try {
                ...
                const matches = files.filter(f => f.startsWith('design-') &&
                    f.endsWith('.md') &&
    ...
    if (explicitMode) {
        ...
        const isLowRisk = inferredChangeType === 'refactor' ||
    ...
    if (sm._savedDesignDoc) {
        const { copyFile: cpFile2 } = await import('node:fs/promises');
        try {
            await cpFile2(sm._savedDesignDoc, join(sm.outputDir, 'design.md'));
    ...
    if (sm._savedProgressLog) {
        ...
        try {
            ...
            const oldCheckpoints = sm._savedProgressLog
                .split('\n')
                .filter((l) => l.includes('<!-- CHECKPOINT'))
                .join('\n');
            if (oldCheckpoints) {
                await writeFile(join(sm.outputDir, 'progress-log.md'), newLog +
    ...
    if (ship === true) {
        ...
        if (deployBranch)
            behaviorUpdates['deployBranch'] = deployBranch;
        if (deployEnv)
            behaviorUpdates['deployEnv'] = deployEnv;
        if (verifyMethod)
            behaviorUpdates['verifyMethod'] = verifyMethod;
        if (verifyConfig)
            behaviorUpdates['verifyConfig'] = verifyConfig;
...
server.tool('auto_dev_checkpoint', "Write structured checkpoint to progress-log and update state.json. Idempotent: same params won't duplicate entries. Atomic: uses write-to-temp-then-rename.", {
    ...
    if (state.step != null) {
        ...
        if (status === 'PASS' &&
            TRIBUNAL_PHASES.includes(phase)) {
            return textResult({
    ...
    if (phase === 1 && status === 'PASS') {
        ...
        if (acContent) {
            ...
            await sm.appendToProgressLog(`<!-- AC_LOCK hash=${hash} total=${stats.total} ` +
...
server.tool('auto_dev_task_red', 'TDD RED phase: validate that only test files were changed and tests fail. Must be called before auto_dev_task_green.', {
    ...
    if (existingState?.status === 'RED_CONFIRMED' ||
        ...
        try {
            ...
            if (!/##\s*Task\s+\d|###\s*Task\s+\d|\d+\./m.test(planContent)) {
                checks.push({
                    name: 'plan_has_tasks',
                    passed: false,
                    message: 'plan.md does not contain recognizable task markers (## Task N or numbered list)',
                });
            }
            else {
                checks.push({
                ...
                try {
                    ...
                    if (crossProjectLessons.length > 0) {
                        ...
                        for (const l of crossProjectLessons) {
                            ...
                            if (l.id)
                                crossProjectLessonIds.push(l.id);
                        }
                        extraContext += '\n';
                    }
                }
...
server.tool('auto_dev_complete', 'Completion gate: validates ALL required phases have PASS status before allowing the session to be declared complete. MUST be called before telling the user that auto-dev is finished. Will REJECT if any phase was skipped.', {
    ...
    const validation = validateCompletion(progressLogContent, state.mode, state.dryRun === true, state.skipE2e === true, state.ship === true);
    ...
    if (buildCmd) {
        try {
            ...
            const buildResult = await new Promise(resolve => {
                exec(buildCmd, { cwd: projectRoot, timeout: 120_000 }, (err, _stdout, stderr) => {
    ...
    if (testCmd) {
        try {
            const { exec } = await import('node:child_process');
            const testResult = await new Promise(resolve => {
                exec(testCmd, { cwd: projectRoot, timeout: 300_000 }, (err, _stdout, stderr) => {
                    if (err && err.code === 'ENOENT') {
    ...
    if (state.worktreeRoot && state.worktreeBranch && state.sourceBranch) {
        ...
        try {
            ...
            const statusOutput = await new Promise((resolve) => {
                execFileGit('git', ['status', '--porcelain'], { cwd: wtRoot, timeout: 10_000 }, (err, stdout) => resolve(err ? '' : stdout || ''));
            });
            // Commit if dirty
            if (statusOutput.trim()) {
                await new Promise((resolve, reject) => {
                    execFileGit('git', ['add', '-A'], { cwd: wtRoot, timeout: 10_000 }, (err) => (err ? reject(err) : resolve()));
                });
                await new Promise((resolve, reject) => {
                    execFileGit('git', ['commit', '-m', 'auto-dev: commit changes before merge'], { cwd: wtRoot, timeout: 10_000 }, (err) => (err ? reject(err) : resolve()));
                });
            }
            // Merge worktree branch to source branch
            await new Promise((resolve, reject) => {
                execFileGit('git', ['merge', wtBranch], { cwd: projectRoot, timeout: 30_000 }, (err) => (err ? reject(err) : resolve()));
            });
            // Remove worktree
            await new Promise((resolve, reject) => {
                execFileGit('git', ['worktree', 'remove', wtRoot], { cwd: projectRoot, timeout: 30_000 }, (err) => (err ? reject(err) : resolve()));
            });
...
// ---------------------------------------------------------------------------
async function promptLessonsFeedbackSimplified(projectRoot, topic) {
    const sm = await StateManager.create(projectRoot, topic);
...
// ===========================================================================
server.tool('auto_dev_tribunal_verdict', 'Submit tribunal verdict from fallback subagent review. Only valid after TRIBUNAL_PENDING.', {
    projectRoot: z.string(),
    topic: z.string(),
    phase: z.number(),
...
// ===========================================================================
server.tool('auto_dev_reset', 'Reset the auto-dev state to a target phase. Useful to manually re-run a phase. Safety guards: cannot reset a COMPLETED project, cannot jump forward, requires a non-empty reason.', {
    projectRoot: z.string(),
    topic: z.string(),
    ...
    const filteredTribunalSubmits = {};
    for (const [k, v] of Object.entries(state.tribunalSubmits ?? {})) {
        if (parseInt(k, 10) < targetPhase)
            filteredTribunalSubmits[k] = v;
    }
    const filteredPhaseEscalateCount = {};
    for (const [k, v] of Object.entries(state.phaseEscalateCount ?? {})) {
        if (parseInt(k, 10) < targetPhase)
            filteredPhaseEscalateCount[k] = v;
    }
