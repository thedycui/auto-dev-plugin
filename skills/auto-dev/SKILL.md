...
0. **必须使用 preflight 返回的 suggestedAgent**。auto-dev 只有 5 个 agent：`auto-dev-architect`（Phase 1+2）、`auto-dev-developer`（Phase 3+5）、`auto-dev-reviewer`（Phase 1-5+7）、`auto-dev-test-architect`（Phase 5）、`auto-dev-acceptance-validator`（Phase 6）。**不存在** `auto-dev-planner`、`auto-dev-planer`、`auto-dev-coder` 等 agent。如果你想调用一个不在上述 5 个之中的 agent，说明你在编造名字——停下来，用 preflight 返回的 `suggestedAgent`。

1. **Phase 必须按顺序执行**：1 → 2 → 3 → 4 → 5 → 6 → 7。Phase 7 不由 auto_dev_complete 自动触发，必须由主 agent 显式 submit。每个 checkpoint/submit 返回值中包含 `mandate` 字段，必须遵守。
...
while phase <= maxPhase:
    ...
    agent_result = 用 result.suggestedPrompt 调用 result.suggestedAgent, model=result.suggestedModel
    tokens += agent_result.total_tokens   # 从 <usage> 标签中提取，累加多个 subagent
...
# ⚠️ Phase 7 不由 auto_dev_complete 自动触发，必须由主 agent 显式 submit。
```

**Tribunal 说明**：
- Phase 4/5/6/7 的 checkpoint(PASS) 调用会被框架拒绝，必须使用 `auto_dev_submit` 代替
- `auto_dev_submit` 会触发独立的裁决 Agent（Tribunal），主 agent 无法影响裁决过程
- 裁决 Agent 独立运行，基于框架数据做出判断，主 agent 的声称不作为依据
...
- `auto_dev_preflight` 返回 `suggestedPrompt`、`suggestedAgent` 和 `suggestedModel`，**必须全部使用**。调用 Agent tool 时传入 `model: result.suggestedModel`。禁止自行覆盖模型选择。
- **`tokenEstimate` 必须传入 checkpoint**。每次 subagent（Agent tool）返回后，从结果中提取 `total_tokens` 值（在 `<usage>` 标签中），累加后传入 `auto_dev_checkpoint(tokenEstimate=N)`。如果一个 Phase 调用了多个 subagent，累加所有 subagent 的 token。这是唯一的 token 统计来源——不传就没有数据。
...
1. 调用 `auto_dev_init(projectRoot, topic, mode, interactive?, dryRun?, skipE2e?, costMode?)` → 获取技术栈和变量
   ...
   - `costMode`：模型选择策略，影响 token 消耗和成本
     - `"beast"`（默认）：全部阶段使用 Opus，最强质量保证
     - `"economy"`：关键阶段用 Opus（设计审查、实现、代码审查），机械性阶段用 Sonnet（计划生成、测试、验收、回顾），**成本约降 50%**
...
> Phase 7 是独立阶段，由 subagent 执行深度审计分析，**不是**主 agent 自己随便写几条经验就完事。
> **禁止主 agent 自己调用 `lessons_add` 后直接 checkpoint PASS。必须调用 reviewer subagent 做完整审计。**

**步骤**:
1. `auto_dev_preflight(phase=7)` → 检查 acceptance-report.md 存在，返回 suggestedPrompt
2. **必须**用 suggestedPrompt 调用 **auto-dev-reviewer** Agent（subagent_type: auto-dev:auto-dev-reviewer）：
   - 阅读 progress-log.md、design.md、design-review.md、plan-review.md、code-review.md、e2e-test-results.md、acceptance-report.md
   - 读取实际代码和测试文件交叉验证（不能只看报告）
   - 执行 **5 维度分析**：诚实度审计、踩坑记录、亮点、流程改进、技术经验
   - 诚实度审计包括：是否跳过阶段、是否被框架拦截、review/测试是否真实、TDD 合规性、是否有作弊行为
   - 对每条经验调用 `auto_dev_lessons_add` 保存
   - **必须**生成 `{output_dir}/retrospective.md`（含诚实度审计表格）
3. 主 agent 确认 `{output_dir}/retrospective.md` 存在且不少于 50 行
4. `auto_dev_checkpoint(phase=7, status="PASS")`
5. **默认模式：PASS 后直接调用 auto_dev_complete，禁止暂停**
...
1. 调用 `auto_dev_complete(projectRoot, topic)` — **必须调用，这是完成门禁**
   ...
   - **Phase 7 不由 auto_dev_complete 自动触发**，必须在调用 complete 之前由主 agent 显式完成 Phase 7 的 submit 流程
...
最精简模式：Phase 3（实现）→ build + test → 完成。
无 tribunal 裁决，仅依赖编译和测试门禁。适用于纯重构、改参数、改文案、fix typo 等真正无风险的小改动（≤20 行、≤2 文件）。
**禁止用于**：涉及类型变更、新增分支逻辑、跨函数影响的改动，即使行数少也必须走 quick。
...
跳过 Phase 5（E2E 测试），保留其他所有 Phase（1→2→3→4→6→7）。适用于小改动、配置修改、纯重构等不需要新增测试文件的场景。

## Cost Mode (--beast / --economy)

控制 subagent 模型选择策略，默认 `economy`。

| 模式 | 参数 | 关键阶段 (1设计/3实现/4审查) | 辅助阶段 (2计划/5测试/6验收/7回顾) | 成本 |
|------|------|---------------------------|--------------------------------|------|
| 疯狂模式 | `--beast`（默认） | Opus | Opus | 100% |
| 经济模式 | `--economy` | Opus | Sonnet | ~50% |

`preflight` 返回 `suggestedModel`，调用 Agent tool 时**必须**传入 `model: result.suggestedModel`。
