...
describe("E2E: self-evolution promotion pipeline", () => {
  ...
  beforeEach(async () => {
    ...
    await mkdir(join(projectRoot, "docs", "auto-dev", "_global"), { recursive: true });
  ...
  it("full pipeline: add → promoteToProject → promoteToGlobal → getCrossProjectLessons", async () => {
    ...
    await mgr.add(3, "technical", "Always validate input before DB writes", undefined, {
      severity: "critical", reusable: true, topic: "auth-feature",
    });
    await mgr.add(3, "pitfall", "Missing null check on optional field", undefined, {
      severity: "important", reusable: true, topic: "auth-feature",
    });
    await mgr.add(3, "process", "Local-only process note", undefined, {
      severity: "minor", reusable: false, topic: "auth-feature",
    });
  ...
  it("cross-project injection: lessons from project A visible in project B", async () => {
    ...
    await mgrA.add(3, "technical", "Cross-project reusable insight", undefined, {
      severity: "critical", reusable: true, topic: "project-a-topic",
    });
  ...
  it("backward compat: old method names still work end-to-end", async () => {
    ...
    await mgr.add(3, "technical", "Compat test lesson", undefined, {
      severity: "important", reusable: true, topic: "compat",
    });
  ...
  it("retrospective integration: promoteToGlobal called during retrospective", async () => {
    ...
    const projectFile = join(projectRoot, "docs", "auto-dev", "_global", "lessons-global.json");
    ...
    await writeFile(join(outputDir, "progress-log.md"), "# Progress\nAll phases passed.\n");
    ...
    const globalEntries = JSON.parse(await readFile(crossGlobalFile, "utf-8")) as LessonEntry[];
