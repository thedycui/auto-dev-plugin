...
async function handleValidationFailure(
  ...
  const prompt = await buildTaskForStep(
    effectiveStep, outputDir, projectRoot, topic, buildCmd, testCmd, combinedFeedback, getExtraVars(effectiveStep), attemptSummary,
  );
...
export async function computeNextTask(
  ...
  const effectiveRoot = (state.worktreeRoot as string | null | undefined) ?? projectRoot;
  ...
  const effectiveCodeRoot = state.worktreeRoot
    ? (state.codeRoot
        ? join(state.worktreeRoot as string, relative(projectRoot, state.codeRoot as string))
