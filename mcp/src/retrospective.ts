...
export async function runRetrospective(
  ...
  for (const m of progressLog.matchAll(
    /CHECKPOINT phase=(\d+).*?status=NEEDS_REVISION/g
  )) {
    await lessons.add(
      Number(m[1]),
      'pitfall',
      `Phase ${m[1]} required revision`,
      undefined,
      {
        severity: 'important',
        reusable: true,
  ...
  for (const m of progressLog.matchAll(
    /CHECKPOINT phase=(\d+).*?status=BLOCKED/g
  )) {
    await lessons.add(
      Number(m[1]),
      'pitfall',
      `Phase ${m[1]} was blocked`,
      undefined,
      {
        severity: 'critical',
        reusable: true,
  ...
  if (codeReview) {
    for (const m of codeReview.matchAll(/\*\*P0\*\*[:\s]*(.+)/gi)) {
      await lessons.add(
        4,
        'pitfall',
        `Code review P0: ${m[1].trim().slice(0, 200)}`,
        undefined,
        {
          severity: 'critical',
          reusable: false,
    ...
    for (const m of codeReview.matchAll(/\*\*P1\*\*[:\s]*(.+)/gi)) {
      await lessons.add(
        4,
        'pitfall',
        `Code review P1: ${m[1].trim().slice(0, 200)}`,
        undefined,
        {
          severity: 'important',
          reusable: false,
  ...
  if (testResults) {
    for (const m of testResults.matchAll(/FAIL[:\s]*(.+)/gi)) {
      await lessons.add(
        5,
        'pitfall',
        `Test failure: ${m[1].trim().slice(0, 200)}`,
        undefined,
        {
          severity: 'important',
          reusable: false,
  ...
  for (const [phase] of Object.entries(phaseTimings)) {
    if (
      !new RegExp(`CHECKPOINT phase=${phase}.*?status=NEEDS_REVISION`).test(
        progressLog
      )
    ) {
      await lessons.add(
        Number(phase),
        'highlight',
        `Phase ${phase} passed on first attempt`,
        undefined,
        {
          severity: 'minor',
          reusable: false,
  ...
  for (const [phase, timing] of Object.entries(phaseTimings)) {
    if (timing.durationMs && timing.durationMs > 10 * 60 * 1000) {
      ...
      await lessons.add(
        Number(phase),
        'process',
        `Phase ${phase} took ${mins} minutes — consider scope reduction`,
        undefined,
        {
          severity: 'minor',
          reusable: true,
  ...
  try {
    ...
    /* non-critical: global promotion failure should not block retrospective */
  }

  const retrospectivePath = join(outputDir, 'retrospective.md');
  const content = generateRetrospectiveDoc(
    state,
    extracted,
    promoted,
    progressLog
  ...
  return {
    lessonsExtracted: extracted,
    globalPromoted: promoted,
    crossProjectPromoted,
    retrospectivePath,
  };
}

function generateRetrospectiveDoc(
  state: StateJson,
  extracted: number,
  promoted: number,
  progressLog: string
  ...
  for (const [p, t] of Object.entries(timings)) {
    ...
    const revs = (
      progressLog.match(
        new RegExp(`CHECKPOINT phase=${p}.*?status=NEEDS_REVISION`, 'g')
      ) ?? []
    ).length;
  ...
  doc += `\n## 踩坑记录\n\n`;
  const revs = [
    ...progressLog.matchAll(/CHECKPOINT phase=(\d+).*?status=NEEDS_REVISION/g),
  ];
  const blks = [
    ...progressLog.matchAll(/CHECKPOINT phase=(\d+).*?status=BLOCKED/g),
  ];
  if (revs.length === 0 && blks.length === 0) {
    doc += `全部一次通过！\n`;
  } else {
    for (const m of revs) doc += `- Phase ${m[1]} 需要修订\n`;
    for (const m of blks) doc += `- Phase ${m[1]} 被阻塞\n`;
  }
