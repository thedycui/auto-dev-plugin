...
function generateRetrospectiveDoc(state: StateJson, extracted: number, promoted: number, progressLog: string): string {
  ...
  const NAMES: Record<string, string> = { "0": "BRAINSTORM", "1": "DESIGN", "2": "PLAN", "3": "EXECUTE", "4": "VERIFY", "5": "E2E_TEST", "6": "ACCEPTANCE" };
  ...
  if (revs.length === 0 && blks.length === 0) { doc += `全部一次通过！\n`; }
  else { for (const m of revs) doc += `- Phase ${m[1]} 需要修订\n`; for (const m of blks) doc += `- Phase ${m[1]} 被阻塞\n`; }

  doc += `\n## 流程改进建议\n\n`;
  let has = false;
  for (const [p, t] of Object.entries(timings)) {
    if (t.durationMs && t.durationMs > 10 * 60 * 1000) { doc += `- Phase ${p} 耗时 ${Math.round(t.durationMs / 60000)} 分钟\n`; has = true; }
  }
