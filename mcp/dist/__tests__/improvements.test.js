...
describe("AC-6: skipE2e in phase-enforcer", () => {
    ...
    it("validateCompletion accepts without phase 5 when skipE2e=true (full mode)", () => {
        const log = [1, 2, 3, 4, 6].map(p => `<!-- CHECKPOINT phase=${p} status=PASS timestamp=2026-01-01T00:00:00Z -->`).join("\n");
    ...
    it("validateCompletion works with quick+skipE2e (phases 3,4 only)", () => {
        const log = [3, 4].map(p => `<!-- CHECKPOINT phase=${p} status=PASS timestamp=2026-01-01T00:00:00Z -->`).join("\n");
