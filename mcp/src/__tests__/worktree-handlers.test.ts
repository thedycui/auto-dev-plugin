...
vi.mock("../state-manager.js", async (importOriginal) => {
  ...
  return {
    ...
    StateManager: class MockStateManager {
      ...
      async atomicWrite(path: string, content: string) { return mockAtomicWrite(path, content); }
      async init() {}
      ...
      async outputDirExists() { return mockOutputDirExists(); }
      async rebuildStateFromProgressLog() { return mockRebuildStateFromProgressLog(); }
      async isCheckpointDuplicate() { return false; }
    },
...
vi.mock("../git-manager.js", () => ({
  GitManager: class MockGitManager {
    ...
    async getChangedFiles(_opts: unknown) { return []; }
    async diffCheck(_files: unknown, _commit: unknown) { return { ok: true, diff: "" }; }
    async rollback(_commit: unknown, _files: unknown) { return { ok: true }; }
  },
}));
...
vi.mock("../phase-enforcer.js", () => ({
  ...
  computeNextDirective: vi.fn().mockReturnValue({ phaseCompleted: false, nextPhase: 3 }),
...
vi.mock("../orchestrator.js", () => ({
  computeNextTask: vi.fn().mockResolvedValue({ done: false, step: "1a", agent: null, prompt: null }),
  firstStepForPhase: vi.fn().mockReturnValue("1a"),
  validateResetRequest: vi.fn().mockReturnValue({ ok: true }),
}));

vi.mock("../ac-runner.js", () => ({
  runStructuralAssertions: vi.fn().mockResolvedValue({ passed: true, results: [] }),
}));
...
vi.mock("../ac-schema.js", () => ({
  AcceptanceCriteriaSchema: {
    safeParse: vi.fn().mockReturnValue({ success: true, data: { criteria: [] } }),
  },
}));
...
describe("AC-2: auto_dev_complete worktree merge and cleanup", () => {
  beforeEach(() => {
    ...
    mockReadFile.mockImplementation((path: string) => {
      if (path.includes("progress-log")) {
        return Promise.resolve("<!-- CHECKPOINT phase=7 status=PASS summary=\"Done\" -->\n");
      }
    ...
    mockExec.mockImplementation((_cmd: string, _opts: Record<string, unknown>, cb: Function) => {
      cb(null, "", "");
    });
  });
  ...
  it("[AC-2] auto_dev_complete calls git merge then git worktree remove", async () => {
    ...
    const addCall = shellCalls.find(c =>
      c.prog === "git" && c.args.includes("add") && c.args.includes("-A") && c.cwd === worktreeRoot
    );
    // Should have called git commit in worktreeRoot
    const commitCall = shellCalls.find(c =>
      c.prog === "git" && c.args.includes("commit") && c.cwd === worktreeRoot
    );
    // Should have called git merge in projectRoot
    const mergeCall = shellCalls.find(c =>
      c.prog === "git" && c.args.includes("merge") && c.args.includes("auto-dev/test-topic")
    );
    // Should have called git worktree remove
    const removeCall = shellCalls.find(c =>
      c.prog === "git" && c.args.includes("worktree") && c.args.includes("remove")
    );
    ...
    expect(mockAtomicUpdate).toHaveBeenCalledWith(expect.objectContaining({ worktreeRoot: null }));
  });
  ...
  it("[AC-2] auto_dev_complete skips commit when worktree is clean", async () => {
    ...
    const addCall = shellCalls.find(c =>
      c.prog === "git" && c.args.includes("add") && c.cwd === worktreeRoot
    );
    expect(addCall).toBeUndefined();

    // Should still merge and remove
    const mergeCall = shellCalls.find(c =>
      c.prog === "git" && c.args.includes("merge")
    );
    const removeCall = shellCalls.find(c =>
      c.prog === "git" && c.args.includes("worktree") && c.args.includes("remove")
    );
...
describe("AC-12: worktree resume — reuse or rebuild", () => {
  ...
  it("[AC-12] resume reuses existing worktree when worktreeRoot dir still exists", async () => {
    ...
    const addCall = shellCalls.find(c =>
      c.prog === "git" && c.args.includes("worktree") && c.args.includes("add") && !c.args.includes("--detach")
    );
    expect(addCall).toBeUndefined();
  });
